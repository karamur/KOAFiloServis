...
### Puantaj Geçişi (MultiDb → v1)
- **`KurumPuantaj.razor`** eklendi (`/kurum-puantaj`) — MultiDb’deki kurum bazlı günlük puantaj ekranı v1’e taşındı.
- **`IKurumPuantajService` + `KurumPuantajService`** eklendi — kurum/güzergah/araç şablonu üretimi, tekil/toplu puantaj upsert, soft-delete akışları taşındı.
- **`Program.cs` DI** güncellendi — `IKurumPuantajService` kaydı eklendi.
- **`NavMenu.razor`** güncellendi — Puantaj menüsü `kurum-puantaj` rotasına yönlendirildi.
- Eski hakediş ekranları projeden kaldırıldı: `FiloHakedisPage.razor`, `HakedisTablosu.razor`, `OperasyonelHakedis.razor`.
- UX iyileştirmesi: KurumPuantaj ilk açılışta ilk aktif kurumu otomatik seçip veriyi yüklüyor; tek kayıt kaydet sonrası değişiklik izleme temizliği düzeltildi.
